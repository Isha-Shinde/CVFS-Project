...
//  Description :       It is used to open an 
//                      existing file
//  Input :             Name of file & Permissions
             ...
             )
{
    ...
    // Check permission
    if(temp->Permission != mode && temp->Permission != 3)
    {
        return ERR_PERMISSION_DENIED;
    }
    
    // Search free UFDT entry
    for(i = 3; i < MAXINODE; i++)
    {
        if(uareaobj.UFDT[i] == NULL)
        {
...
//  Description :       It is used to closed 
//                      already opend file
...
//  Description :       It is used to change the
//                      current file offset
//  Input :             File Descriptor
//                      Offset
              ...
              )
{
    ...
    if(from == START)
    {
        ...
        uareaobj.UFDT[fd]->ReadOffset = offset;
    }
    else if(from == CURRENT)
    {
        if((uareaobj.UFDT[fd]->WriteOffset + offset < 0) || (uareaobj.UFDT[fd]->WriteOffset + offset > MAXFILESIZE))
        {
            return ERR_INVALID_PARAMETER;
        }
        uareaobj.UFDT[fd]->ReadOffset += offset;
    ...
    {
        if((uareaobj.UFDT[fd]->ptrinode->ActualFileSize + offset < 0) ||(uareaobj.UFDT[fd]->ptrinode->ActualFileSize + offset > MAXFILESIZE))
        {
            return ERR_INVALID_PARAMETER;
        }
        uareaobj.UFDT[fd]->ReadOffset = uareaobj.UFDT[fd]->ptrinode->ActualFileSize + offset;
    }
    else
    {
        return ERR_INVALID_PARAMETER;
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            else if(strcmp(Command[0],"close") == 0)
            {
                ...
                {
                    printf("file gets closed successfully\n");
                }
            }
            else
            {
        ...
        else if(iCount == 3)
        {
            ...
            else if(strcmp(Command[0], "open") == 0)
            {
                ...
                {
                    printf("Error : Maximim files are open. close some files\n");
                }
                else if(iRet == ERR_PERMISSION_DENIED)
                {
                    printf("Error : Permission denied\n");
                ...
                {
                    printf("open operation is succesful\n");
                }
            }
            else
            {
        ...
        else if(iCount == 4)
        {
            ...
            if(strcmp(Command[0], "lseek") == 0)
            {
                ...
                {
                    printf("lseek operation is succesful\n");
                }
            }
        }
        else
